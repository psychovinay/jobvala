...
const TalentCard = (props: any) => {
    ...
    const handleOffer = (status:string) => {
        // Use the applicantId coming from the applicant data (props), not the profile id
        let payload:any={ id: Number(id), applicantId: Number(props?.applicantId), applicationStatus:status };
        if(status==="INTERVIEWING"){
            if(!date || !time){
                errorNotification('Error', 'Please select both date and time');
                return;
            }
            const [hours, minutes] = String(time).split(':').map((v)=>Number(v));
            const dt = new Date(date);
            dt.setHours(hours||0);
            dt.setMinutes(minutes||0);
            const isoLocal = dt.toISOString().slice(0,19); // 'YYYY-MM-DDTHH:mm:ss'
            payload={...payload, interviewTime: isoLocal };
        }
        changeAppStatus(payload).then((res) => {
            ...
            console.log(err)
            errorNotification('Error', err?.response?.data?.errorMessage || 'Could not update application status');
        });
